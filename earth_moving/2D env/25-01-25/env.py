...
class SimulationEnv:
    ...
    #
    #         # Check if a cell already exists at this location
    #         existing_cell = next((cell for cell in self.cells_with_objects if cell.x == cell_x and cell.y == cell_y),
    #                              None)
    #         if existing_cell:
    #             # Increment the object count for the cell
    #             existing_cell.num_objects += 1
    ...
    def find_best_path(self, start_cell):
        """
        Find the best path from the given cell to the target zone.
        Neighbors must be closer to the target zone and within a valid range.
        """
        visited = set()
        queue = [(0, start_cell, [])]  # (priority, current cell, path so far)
        best_path = []
        max_objects = 0

        while queue:
            # Get the cell with the lowest priority
            priority, current_cell, path = queue.pop(0)
            ...
            if current_cell.distance_to_target <= self.target_zone_radius:
                total_objects = sum(cell.num_objects for cell in new_path)
            ...
            for neighbor_info in current_cell.visible_cells:
                neighbor = neighbor_info["cell"]
                if neighbor not in visited:
                    # Ensure neighbor is closer to the target zone
                    if neighbor.distance_to_target < current_cell.distance_to_target:
                        # Ensure the distance between current cell and neighbor is valid
                        distance_to_neighbor = math.sqrt(
                            (neighbor.x - current_cell.x)**2 + (neighbor.y - current_cell.y)**2
                        )
                        if distance_to_neighbor <= current_cell.distance_to_target:
                            # Use negative object count for priority to maximize objects
                            neighbor_priority = priority - neighbor.num_objects
                            queue.append((neighbor_priority, neighbor, new_path))
            # Sort the queue based on priority (first element of the tuple)
    ...
    def calculate_potential_field(self):
        ...
        for cell in self.cells_with_objects + self.cells_without_objects:
            ...
            #
            # # Calculate potential based on max_objects and distance to target
            # if best_path:
            #     cell.potential = cell.distance_to_target - cell.total_objects_target
            # else:
            #     cell.potential = cell.distance_to_target  # Default to distance if no objects collected
    ...
    def update_heat_map(self):
        ...
        self.highway_threshold = max_potential * 0.5  # Example: 80% of the max potential
    ...
    def update_local_potential(self, affected_cells):
        """Update potential for cells affected by changes."""
        for cell in affected_cells:
            cell.calculate_potential()

    def calculate_visibility_for_all_cells(self):
        """Calculate visibility for all cells in the grid."""
        for cell in self.all_cells:
            self.calculate_visibility_simple(cell)  # Use your simplified visibility calculation
    ...
    def calculate_visibility(self, cell, triangle):
        ...
        for other_cell in self.cells_with_objects:
            ...
            # Debugging output
        # print(f"Cell({cell.x}, {cell.y}): {len(cell.visible_cells)} visible cells")
