...
from visual_color_utils import pygame_rgb
...
class SimulationVisualizer:
    def __init__(self, env, screen_size=500):
        ...
        self.agent_colors_by_id = {}  # Exact colors supplied by the 3D rover configuration.
        self.agent_colors = [
            (255, 0, 0),    # Red
            (0, 0, 255),    # Blue  
            (0, 255, 0),    # Green
    ...
    def draw_target_zone(self):
        """Draw the actual target polygon used by planning."""
        geometry = self.env.target_zone
        polygons = list(geometry.geoms) if hasattr(geometry, "geoms") else [geometry]
        for polygon in polygons:
            points = [
                (float(x) * self.cell_size, float(y) * self.cell_size)
    ...
    def draw_velocity_highway(self):
        ...
        for cell in self.env.get_all_cells_with_objects():
            # ג… Derive direction from best_path_highway instead of velocity_highway
    ...
    def set_trajectory_preview(self, start_cell, path_type, path_info, agent_id=None, agent_color=None):
        """
        ...
        self.preview_agent_id = agent_id
        self.preview_color = pygame_rgb(agent_color) if agent_color is not None else None
    ...
    def draw_trajectory_preview(self):
        ...
        if start_cell:
            ...
            pygame.draw.rect(self.screen, self.preview_color or (255, 255, 0), start_rect, 4)

        # Draw path cells with colored borders
        path_color = self.preview_color or ((0, 255, 0) if self.preview_path_type == 'target' else (255, 165, 0))
    ...
    def draw_agents(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            x, y = agent.position
            agent_id = getattr(agent, "agent_id", i)
            ...
            color = pygame_rgb(
                getattr(agent, "color", None),
                self.agent_colors_by_id.get(agent_id, self.agent_colors[i % len(self.agent_colors)]),
            )
            ...
            text = font.render(str(agent_id), True, (0, 0, 0))
    ...
    def draw_agent_paths(self):
        ...
        for path_order, (agent_id, path_points) in enumerate(self.agent_paths.items()):
            if not path_points or len(path_points) < 2:
                continue
                
            color = pygame_rgb(
                self.agent_colors_by_id.get(agent_id),
                self.agent_colors[path_order % len(self.agent_colors)],
            )
    ...
    def update_agent_colors(self, agent_colors_dict):
        """Use the exact configured PyBullet color for every PyGame rover and path."""
        self.agent_colors_by_id = {
            agent_id: pygame_rgb(color)
            for agent_id, color in (agent_colors_dict or {}).items()
        }
    def run(self):
