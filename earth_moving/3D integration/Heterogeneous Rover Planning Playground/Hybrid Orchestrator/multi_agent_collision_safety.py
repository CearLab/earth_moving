...
def _pair_r_sum(a: AgentSnapshot, b: AgentSnapshot, config: PolicyConfig) -> float:
    return max(float(config.conflict_r_sum), float(a.collision_radius) + float(b.collision_radius))

def _extra_radius(value: float) -> float:
    return max(0.0, float(value) - 0.46)
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if are_on_collision_course(
                ...
                r_sum=_pair_r_sum(snapshots[i], snapshots[j], config),
            ):
                adj[i].append(j)
                adj[j].append(i)
    ...
    def _choose_episode(self, contexts: Sequence[SafetyAgentContext]) -> Optional[ClusterEpisode]:
        ...
        yielder_locals = [
            ...
            if i != winner_local and orca_sees_collision_potential(
                ...
                r_sum=_pair_r_sum(snapshots[winner_local], snapshots[i], self.config),
            )
        ]
    ...
    def _path_winner_can_release(self, contexts: Sequence[SafetyAgentContext]) -> bool:
        ...
        for y_idx in self.episode.yielder_indices:
            ...
            if y_ctx is None or y_pred is None:
                continue
            clearance = self.config.path_clearance_radius + _extra_radius(winner_ctx.collision_radius) + _extra_radius(y_ctx.collision_radius)
            if distance_to_path((float(y_ctx.state[0]), float(y_ctx.state[1])), winner_ctx.path_points) < clearance:
                return False
            if orca_sees_collision_potential(
                winner_pred,
                y_pred,
                tau=self.config.conflict_tau,
                r_sum=_pair_r_sum(winner_pred, y_pred, self.config),
            ):
                return False
        return True
            ...
            if winner.path_constrained:
                ...
                can_release = winner_pred is not None and not any(
                    y_idx in snapshots and orca_sees_collision_potential(
                        ...
                        snapshots[y_idx],
                        tau=self.config.conflict_tau,
                        r_sum=_pair_r_sum(winner_pred, snapshots[y_idx], self.config),
                    )
                    for y_idx in self.episode.yielder_indices
                )
        ...
        for a in contexts:
            for b in contexts:
                ...
                emergency_distance = (
                    ...
                    + _extra_radius(a.collision_radius)
                    + _extra_radius(b.collision_radius)
                )
