...
from spillage_model import simulate_spillage,smooth_path_with_spline


class SimulationEnv:
    def __init__(self, grid_size, target_zone_radius=10, agent_positions=None, num_random_objects=0, seed=None, 
                 real_objects=None, use_real_objects=False, real_world_size=None, shovel_size=None, 
                 manual_mode=False):
        ...
        self.use_real_objects = use_real_objects
        self.manual_mode = manual_mode
        
        # Calculate scaling factors only in real-time mode
        if use_real_objects and real_world_size and shovel_size and not manual_mode:
            # Calculate how many cells should represent the shovel
            self.cells_per_shovel = 2  # We want the shovel to be represented by 2 cells
            # Calculate the scaling factor from real-world to grid coordinates
            self.scale_factor = (self.cells_per_shovel / shovel_size) * self.grid_size / real_world_size
            
            # Print scaling information
            print("\nScaling Information:")
            print(f"Grid size: {grid_size}x{grid_size}")
            print(f"Real world size: {real_world_size} meters")
            print(f"Shovel size: {shovel_size} meters")
            print(f"Cells per shovel: {self.cells_per_shovel}")
            print(f"Scale factor: {self.scale_factor}")
            print(f"Each grid cell represents: {real_world_size/grid_size:.3f} meters")
            print(f"Each meter represents: {grid_size/real_world_size:.3f} grid cells")
        else:
        ...
        # Handle object placement based on input type
        if use_real_objects and real_objects:
            self._place_real_objects(real_objects)
        else:
    ...
    def _place_real_objects(self, real_objects):
        """Place real objects in the environment."""
        for x, y in real_objects:  # x, y are already in grid coordinates
            # Ensure coordinates are within grid bounds
            if 0 <= x < self.grid_size and 0 <= y < self.grid_size:
                cell_center = Point(x + 0.5, y + 0.5)  # Center of the grid cell
                ...
                # Check if a cell already exists at this location
                existing_cell = next(
                    (cell for cell in self.cells_with_objects if cell.x == x and cell.y == y),
                    None
                )
                if existing_cell:
                    # Increment the object count for the cell
                    ...
                    # Add a new cell with one object
                    new_cell = Cell(x, y, 1, self.target_zone, self.grid_size)
                    self.cells_with_objects.append(new_cell)

            else:
                print(f"Warning: Object at ({x}, {y}) is out of bounds")
        ...
        for cell in sorted_cells:
            ...
            for path_info in best_paths:
                ...
                if use_spillage_model:
                    # Calculate spline points for the path
                    waypoints = [(c.x, c.y) for c in path]
                    spline_points, curvature, ok = smooth_path_with_spline(waypoints)
                    
                    if ok:
                        # Store spline points and curvature
                        cell.spline_points_target = spline_points
                        cell.curvature_target = curvature
                    ...
                    # Calculate spline points even without spillage model
                    waypoints = [(c.x, c.y) for c in path]
                    spline_points, curvature, ok = smooth_path_with_spline(waypoints)
                    if ok:
                        cell.spline_points_target = spline_points
    ...
    def calculate_path_to_highway(self, use_spillage_model=False):
        ...
        for cell in self.cells_with_objects:
            ...
            # Calculate visibility for this candidate
            ...
            for path_info in best_paths:
                ...
                if use_spillage_model:
                    ...
                    if ok:
                        # Store spline points and curvature
                        cell.spline_points_highway = spline_points
                        cell.curvature_highway = curvature
                        
                        # Estimate objects reaching the highway
                        _, impacted_cells, estimated_objects, _ = simulate_spillage(
                            waypoints=waypoints,
                            objects_at_cells={(c.x, c.y): c.num_objects for c in path if c.num_objects > 0},
                            agent_capacity=self.agent_capacity,
            ...
            cell.impacted_cells_highway = best_impacted_cells
    ...
    def update_real_objects(self, real_objects):
        """Update the environment with new real object positions."""
        ...
        self.use_real_objects = True
        
        # Recalculate cell states
        self.calculate_cell_states()
    ...
    def get_trajectory(self, cell, path_type, use_spillage=False):
        """Calculate trajectory points for a given cell and path type.
        
        Args:
            cell (Cell): The cell to calculate trajectory from
            path_type (str): Either 'target' or 'highway'
            use_spillage (bool): Whether to use spillage model
            
        Returns:
            list: List of (x, y) coordinates representing the trajectory
        """
        if path_type == 'target':
            # Use stored spline points if available
            if cell.spline_points_target:
                return cell.spline_points_target
            
            # If no spline points stored, calculate them
            waypoints = [(c.x, c.y) for c in cell.best_path_target]
            spline_points, curvature, ok = smooth_path_with_spline(waypoints)
            if ok:
                cell.spline_points_target = spline_points
                cell.curvature_target = curvature
            ...
            # Use stored spline points if available
            if cell.spline_points_highway:
                return cell.spline_points_highway
            
            # If no spline points stored, calculate them
            waypoints = [(c.x, c.y) for c in cell.best_path_highway]
            spline_points, curvature, ok = smooth_path_with_spline(waypoints)
            if ok:
                cell.spline_points_highway = spline_points
                cell.curvature_highway = curvature
        ...
        # Fallback: if spline calculation fails, return original path points
        if path_type == 'target':
            return [(c.x, c.y) for c in cell.best_path_target]
        else:
            return [(c.x, c.y) for c in cell.best_path_highway]
