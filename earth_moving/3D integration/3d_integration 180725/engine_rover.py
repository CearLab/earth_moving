...
class PyBulletEnvironment:
    ...
    def follow_smooth_trajectory(env, trajectory, control_dt=1/240, lookahead=1):
        """
        ...
        :param env: PyBulletEnvironment instance
        ...
        while not reached and i < max_iter:
            ...
            # Compute velocities
            target_V, target_phi, _ = env.compute_target_velocities(
                target_pos, target_quat, current_pos, current_quat
            )

            # Convert to wheel commands
            omega_r, omega_l = env.compute_wheel_velocities(target_V, target_phi)

            # Apply control
            env.control_rover_velocity(omega_l, omega_r, control_dt)
            i += 1

            # Check if reached final point
            final_pos = trajectory[-1]
            pos_error = np.linalg.norm(np.array(current_pos[:2]) - np.array(final_pos[:2]))
            if pos_error < env.control_threshold:
                reached = True
