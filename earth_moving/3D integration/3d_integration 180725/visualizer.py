...
class SimulationVisualizer:
    def __init__(self, env, screen_size=500):
        ...
        self.current_trajectory = None  # Store current trajectory for visualization
    ...
    def handle_click_event(self, pos):
        ...
        :return: The clicked cell if it has objects, None otherwise.
        """
    ...
    def set_trajectory(self, trajectory):
        """Set the current trajectory for visualization."""
    ...
    def draw_trajectory(self):
        """Draw the current trajectory on the 2D visualization."""
        if not self.current_trajectory:
            return

        # Draw trajectory lines
        for i in range(len(self.current_trajectory) - 1):
            start = self.current_trajectory[i]
            end = self.current_trajectory[i + 1]
            ...
            end_screen = (
                end[0] * self.cell_size,
                end[1] * self.cell_size
            )
        ...
        if self.current_trajectory:
            ...
            last_screen = (
                last_point[0] * self.cell_size,
                last_point[1] * self.cell_size
            )
            pygame.draw.circle(
                self.screen,
                (0, 0, 0),  # Black color
                (int(last_screen[0]), int(last_screen[1])),
                3  # Radius
            )
    ...
    def draw_elements(self):
        ...
        self.draw_trajectory()  # Draw the current trajectory
