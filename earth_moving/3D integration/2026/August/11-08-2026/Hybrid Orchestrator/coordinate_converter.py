...
class CoordinateConverter:
    def __init__(self, env_radius=1.0, target_zone_radius=0.3, shovel_width=0.22,
        ...
        self.target_zone_spec = resolve_target_zone(target_zone, target_zone_radius)
        self.target_zone_radius = self.target_zone_spec.bounding_radius
        ...
        self._target_zone_mask = self._build_target_zone_mask()
        
        print("\nCoordinate Converter Initialized:")
        print(f"Environment radius: {self.env_radius:.3f}m")
        print(f"Target zone: {self.target_zone_spec.shape} {self.target_zone_spec.to_dict()}")
    ...
    def _build_target_zone_mask(self):
        """Cache target membership for the canonical grid used by overlays."""
        if self.target_zone_spec.shape == "polygon":
            # Shapely 2 performs this entire irregular-polygon query in C.
            import numpy as np
            from shapely import intersects_xy
            coordinates = np.arange(self.grid_size, dtype=float)
            grid_x, grid_y = np.meshgrid(coordinates, coordinates)
            world_x = grid_x * self.cell_size - self.env_radius
            world_y = self.env_radius - grid_y * self.cell_size
            mask = intersects_xy(self.target_zone_spec.geometry, world_x, world_y)
            return np.asarray(mask, dtype=np.uint8).tobytes()
        mask = bytearray(self.grid_size * self.grid_size)
        for grid_y in range(self.grid_size):
            row_offset = grid_y * self.grid_size
            for grid_x in range(self.grid_size):
                x, y = self.convert_2d_to_3d(grid_x, grid_y)
                mask[row_offset + grid_x] = int(self.target_zone_spec.contains_world(x, y))
        return bytes(mask)
    def is_in_target_zone(self, grid_x, grid_y):
        """Return cached exact target membership for a canonical grid cell."""
        grid_x, grid_y = int(grid_x), int(grid_y)
        if not (0 <= grid_x < self.grid_size and 0 <= grid_y < self.grid_size):
            return False
        return bool(self._target_zone_mask[grid_y * self.grid_size + grid_x])
    def distance_to_target_zone(self, x, y):
        return self.target_zone_spec.distance_world(x, y)

    def closest_point_on_target_zone(self, x, y):
        return self.target_zone_spec.closest_point_world(x, y)

    def get_target_zone_grid_polygon(self):
        return self.target_zone_spec.grid_polygon(self.grid_size, self.cell_size)
    ...
    def get_environment_info(self):
        ...
        return {
            ...
            'target_zone': self.target_zone_spec.to_dict(),
