...
def calculate_target_ready_fraction(snapshot, overlay, policy, root_sources_only=False):
    ...
    for candidate in overlay.candidates:
        if candidate.task_type != TARGET_TASK:
            continue
        if root_sources_only and not candidate.is_root_source:
            continue
