...
from shapely import (
    contains_xy, get_x, get_y, line_interpolate_point, line_locate_point, points,
)
import numpy as np
...
class SimulationEnv:
    def __init__(
        self, grid_size, target_zone_radius=10, agent_positions=None,
        num_random_objects=0, seed=None, max_path_length_factor=2.5,
        target_angle_tolerance=30, highway_angle_tolerance=60,
        highway_min_heat_ratio=0.2, highway_threshold_ratio=0.5,
        highway_heat_weight=0.7, highway_distance_weight=0.3,
        target_zone=None, target_zone_metadata=None,
        target_visibility_mode="fixed", highway_visibility_mode="fixed",
        dynamic_visibility_min_angle=30, dynamic_visibility_max_angle=60,
        target_zone_gate_factor=1.0,
    ):
        ...
        # ג… HIGHWAY TARGET SELECTION CONFIGURATION
        ...
        # Visibility can use a fixed cone or a distance-adjusted cone.
        valid_visibility_modes = {"fixed", "dynamic"}
        if target_visibility_mode not in valid_visibility_modes:
            raise ValueError("target_visibility_mode must be 'fixed' or 'dynamic'")
        if highway_visibility_mode not in valid_visibility_modes:
            raise ValueError("highway_visibility_mode must be 'fixed' or 'dynamic'")
        self.target_visibility_mode = target_visibility_mode
        self.highway_visibility_mode = highway_visibility_mode
        ...
        self.angle_min_deg = float(dynamic_visibility_min_angle)
        self.angle_max_deg = float(dynamic_visibility_max_angle)
        if not 0.0 < self.angle_min_deg <= self.angle_max_deg <= 180.0:
            raise ValueError("dynamic visibility angles must satisfy 0 < min <= max <= 180")
        self.target_zone_gate_factor = float(target_zone_gate_factor)
        if self.target_zone_gate_factor <= 0.0:
            raise ValueError("target_zone_gate_factor must be positive")
        ...
        self.target_zone_metadata = dict(target_zone_metadata or {})
    ...
    def target_visibility_angle(self, current_cell=None):
        """Fixed degrees, or None to let visibility compute the dynamic angle."""
        if self.target_visibility_mode == "dynamic":
            return None
        return float(self.target_angle_tolerance)

    def highway_visibility_angle(self, current_cell=None, target_cell=None):
        """Fixed degrees, or None to let visibility compute the dynamic angle."""
        if self.highway_visibility_mode == "dynamic":
            return None
        return float(self.highway_angle_tolerance)

    def _precompute_target_metrics(self):
        """Vectorize target membership and nearest-boundary data for every cell."""
        coordinates = np.arange(self.grid_size, dtype=float) + 0.5
        grid_x, grid_y = np.meshgrid(coordinates, coordinates, indexing="ij")
        flat_x, flat_y = grid_x.ravel(), grid_y.ravel()
        cell_points = points(flat_x, flat_y)
        boundary = self.target_zone.exterior
        locations = line_locate_point(boundary, cell_points)
        nearest = line_interpolate_point(boundary, locations)
        closest_x = np.asarray(get_x(nearest), dtype=float).reshape(grid_x.shape)
        closest_y = np.asarray(get_y(nearest), dtype=float).reshape(grid_y.shape)
        distances = np.hypot(closest_x - grid_x, closest_y - grid_y)
        inside = np.asarray(
            contains_xy(self.target_zone, flat_x, flat_y), dtype=bool
        ).reshape(grid_x.shape)
        return inside, closest_x, closest_y, distances

    def _initialize_cells(self):
        """Initialize all cells using one vectorized target-geometry pass."""
        inside, closest_x, closest_y, distances = self._precompute_target_metrics()
        existing_by_xy = {(cell.x, cell.y): cell for cell in self.cells_with_objects}
        for x in range(self.grid_size):
            for y in range(self.grid_size):
                target_metrics = (
                    inside[x, y], closest_x[x, y], closest_y[x, y], distances[x, y])
                existing_cell = existing_by_xy.get((x, y))
                if existing_cell is not None:
                    existing_cell.apply_target_metrics(target_metrics)
                    self.all_cells.append(existing_cell)
                else:
                    empty_cell = Cell(
                        x, y, 0, self.target_zone, self.grid_size,
                        target_metrics=target_metrics)
                    self.cells_without_objects.append(empty_cell)
                    self.all_cells.append(empty_cell)

        self.cells_by_xy = {(cell.x, cell.y): cell for cell in self.all_cells}

    def _set_target_zone(self):
        """Set the legacy circular target zone in the center of the grid."""
    ...
    def _add_real_objects(self, objects_2d):
        ...
        for grid_x, grid_y in objects_2d:
            ...
            if canonical_cell.is_target_zone:
    ...
    def nearest_target_terminal_cell(self, current_cell):
        """Return a real grid cell whose center is inside the target polygon.

        Polygon boundary coordinates are continuous.  Truncating a closest
        boundary point can map back to the same *outside* grid cell, creating a
        self-loop instead of a terminal A* edge.  Cache all verified target
        cells and select the nearest one by center-to-center distance.
        """
        terminals = getattr(self, "_target_terminal_cells", None)
        if terminals is None:
            terminals = tuple(
                cell for cell in self.all_cells
                if bool(getattr(cell, "is_target_zone", False))
            )
            self._target_terminal_cells = terminals
        if not terminals:
            return None
        ...
        return min(
            terminals,
            key=lambda cell: (
                (cell.x + 0.5 - sx) ** 2 + (cell.y + 0.5 - sy) ** 2,
                int(cell.x),
                int(cell.y),
            ),
        )
    ...
    def calculate_potential_field(self, use_spillage_model=True, visualize=True, affected_cells=None):
        ...
        # ג… Clear flow tracking values before A* runs (will be updated during search)
        ...
        for cell in sorted_cells:
            ...
            # ג… Store path and estimated results
            ...
            cell.impacted_cells_target = best_impacted_cells  # ג… Store impacted cells!
            
            # ג… Mark cell as solved and compute exact heuristic (spillage OFF only)
        ...
        # ג… Run global propagation for initial calculations (needed for heat map)
    ...
    def calculate_path_to_highway(self, use_spillage_model=False):
        ...
        # ג… PERFORMANCE OPTIMIZATION: Calculate candidate pool once outside the loop
        ...
        for cell in self.cells_with_objects:
            # ג… Skip cells already in high-potential locations
            if cell.heat_map >= self.highway_threshold:
                continue

            # ג… SIMPLE DISTANCE CONSTRAINT: Find highest heat cell within distance_to_target
            ...
            # ג… HYBRID SCORING: Combine heat map and distance with configurable weights
            ...
            # ג… Store the originally chosen target (before A* might change it)
            cell.chosen_highway_target = target
            
            # ג… STEP 2: Calculate visibility toward the SPECIFIC chosen target
            cell.visible_cells_highway, cell.distance_to_children_highway = self.calculate_highway_visibility(
                cell, target, angle_tolerance=self.highway_visibility_angle(cell, target)
            )

            # ג… STEP 3: Find the best path to the SPECIFIC highway target
            ...
            # ג… Select the **best path** among candidates
            best_path = None
            max_objects = 0
            best_distance = float("inf")
            best_impacted_cells = {}
            ...
            for path_info in best_paths:
                ...
                if use_spillage_model:
                    # ג… Estimate objects reaching the highway **considering spillage**
                    _, impacted_cells, estimated_objects, _ = simulate_spillage(
                        waypoints=[(c.x, c.y) for c in path],
                        objects_at_cells={(c.x, c.y): c.num_objects for c in path if c.num_objects > 0},
                        agent_capacity=self.agent_capacity,
                ...
                # ג… Choose the best path considering estimated objects & distance
                if estimated_objects > max_objects or (
                        estimated_objects == max_objects and total_distance < best_distance):
            ...
            # ג… Assign the best found path
            cell.best_path_highway = best_path
            cell.total_objects_highway = max_objects
            cell.total_distance_highway = best_distance
            cell.impacted_cells_highway = best_impacted_cells  # ג… Store impacted cells!

            # ג… Compute velocity direction for the agent
            if len(best_path) > 1:
    ...
    def _initialize_spillage_cell(self, spillage_cell):
        ...
        visible_cells_target, distance_to_children_target = self.calculate_target_zone_visibility(
            spillage_cell, angle_tolerance=self.target_visibility_angle(spillage_cell)
        )
    ...
    def execute_path(self, start_cell, path_type, use_spillage=True, precomputed_path=None):
        ...
        # ג… Ensure a valid path exists
        if not best_path or len(best_path) < 2:
            print(f"WARNING: No valid path found for cell ({start_cell.x}, {start_cell.y}) with path type '{path_type}'.")
            return

        # ג… Track affected cells (for backward propagation later)
        affected_cells = set()
        
        # ג… Track objects for conservation verification
        ...
        if use_spillage and impacted_cells:
            ...
            # ג… **Move all objects to the last cell, clearing intermediate ones**
            ...
            # ג… Ensure final cell is tracked in cells_with_objects
            ...
            # ג… Remove objects from all path cells (except the final cell)
            for cell in best_path[:-1]:  # Exclude final cell
                cell.num_objects = 0
                cell.current_objects = 0
                # ג… Clear both target and highway path cache for zeroed cells
                cell.best_path_target = []
                cell.next_child_target = None
                cell.total_objects_path_target = 0
                cell.total_objects_target = 0
        ...
        # ג… Ensure cells_with_objects tracking is consistent
        ...
        # ג… Store affected cells for `update_environment`
        self.affected_cells = affected_cells
        
        # ג… Store spillage-specific affected cells for visualization
        if use_spillage and impacted_cells:
    ...
    def update_environment(self):
        ...
        # ג… Ensure cells_with_objects tracking is consistent before recalculation
        ...
        # ג… Separate different types of affected cells
        ...
        # ג… Add spillage cells (these always have objects by definition)
        spillage_cells = set()
        ...
        # ג… Filter out target zone cells - they don't need pathfinding processing
        ...
        # ג… Find recalculation cells: cells that can see ANY affected cell (including emptied ones)
        ...
        # ג… Combine for total affected cells  
        all_affected_cells = all_direct_affected | recalculation_cells
        
        # ג… Store different types for visualization
        ...
        # ג… STEP 0: Invalidate solved flags for affected cells (spillage OFF only)
        ...
        # ג… STEP 1: Refresh visibility for affected cells using current truth
        ...
        for cell in all_affected_cells:
            if cell.num_objects > 0:  # Only refresh visibility for cells that still have objects
                cell.visible_cells_target, cell.distance_to_children_target = \
                    self.calculate_target_zone_visibility(
                        cell, angle_tolerance=self.target_visibility_angle(cell)
                    )
        ...
        # ג… STEP 2: Recalculate target A* paths for cells with objects only (uses fresh visibility)
        ...
        # ג… STEP 3: Clear stale propagation values and propagate target paths
        ...
        # ג… STEP 4: Update velocity field (uses fresh target paths) - all affected cells
        print("Updating velocity field for all affected cells...")
        self.calculate_velocity_field(affected_cells=all_affected_cells)

        # ג… STEP 5: Heat map recalculation (uses propagated target data)
        print("Updating heat map...")
        self.update_heat_map()

        # ג… STEP 6: Recalculate highway paths (uses updated heat map)
        ...
        # ג… STEP 7: Remove cells that no longer contain objects AFTER all calculations
        ...
        # ג… Orphan cell diagnostics - ensure all cells are properly tracked
    ...
    def in_cone_and_gate(self, observer, neighbor, target_cell=None):
        ...
        """
        angle_tolerance = self.target_visibility_angle(observer)
        if angle_tolerance is None:
            angle_tolerance = self.get_angle_tolerance_deg(observer, target_cell)
        ...
        if target_cell is None:
            ...
            angle_tolerance = self.highway_visibility_angle(observer, target_cell)
            if angle_tolerance is None:
                angle_tolerance = self.get_angle_tolerance_deg(observer, target_cell)
        ...
        # ג… CONE CONSTRAINT: Angular tolerance check
        dot = dx * vx + dy * vy
        in_cone = dot >= math.sqrt(candidate_dist_sq * L2) * cos_th
        
        # ג… GATE CONSTRAINT: Distance within gate factor
        distance_to_candidate = math.sqrt(candidate_dist_sq)
        within_gate = distance_to_candidate <= gate_distance
        
        # ג… PROGRESS CONSTRAINT: Monotone progress toward target (target zone mode only)
        if target_cell is None:
    ...
    def is_geometrically_visible_to_target(self, observer_cell, candidate_cell, angle_tolerance=None):
        ...
        """
        # Use configured fixed/dynamic target visibility if not specified.
        if angle_tolerance is None:
            angle_tolerance = self.target_visibility_angle(observer_cell)
        if angle_tolerance is None:
            angle_tolerance = self.get_angle_tolerance_deg(observer_cell)
        ...
        # ג… CONE CONSTRAINT: Angular tolerance check (same math as original)
        dot = dx * vx + dy * vy
        in_cone = dot >= math.sqrt(candidate_dist_sq * L2) * cos_th
        
        # ג… GATE CONSTRAINT: Distance within gate factor (same logic)
        distance_to_candidate = math.sqrt(candidate_dist_sq)
        within_gate = distance_to_candidate <= observer_cell.closest_distance * self.target_zone_gate_factor
        
        # ג… PROGRESS CONSTRAINT: Monotone progress toward target (same constraint)
    ...
    def is_cell_in_visibility_scope(self, observer_cell, target_cell):
        ...
        """
        # ג… FAST PATH: Check pre-computed visibility list first (for cells that had objects during calculation)
        ...
        # ג… FALLBACK PATH: Use consistent cone and gate check (matches actual visibility calculations)
    ...
    def calculate_visibility_simple(self, current_cell, angle_tolerance=None, target_cell=None):
        ...
        for neighbor in self.cells_with_objects:
            ...
            if target_cell:
                ...
                # No overshoot: dֲ²(S,N) ג‰₪ dֲ²(S,T) (using squared distances)
                no_overshoot = neighbor_dist_sq <= L2
                
                # Monotone progress: dֲ²(N,T) < dֲ²(S,T) (using squared distances)
        ...
        if not visible_cells and not target_cell:
            terminal_cell = self.nearest_target_terminal_cell(current_cell)
            if terminal_cell is not None and terminal_cell is not current_cell:
                distance = math.hypot(
                    terminal_cell.x - current_cell.x,
                    terminal_cell.y - current_cell.y,
                )
                visible_cells.append({"cell": terminal_cell})
                distance_to_children[(terminal_cell.x, terminal_cell.y)] = distance
    ...
    def calculate_target_zone_visibility(self, current_cell, angle_tolerance=None):
        ...
        # Fallback: connect to a verified inside-target terminal.  Never use
        # int(closest_boundary), which can truncate back to current_cell for
        # off-center, concave, or thin polygon targets.
    ...
    def audit_visibility(self):
        ...
        for c in self.all_cells:
            # Target membership was precomputed when the canonical cell grid was built.
            inside_target = bool(c.is_target_zone)
        ...
        if self.dead_visibility_target or self.dead_visibility_highway:
            print(
                f"[Visibility Audit] Dead cells ג€” target:{len(self.dead_visibility_target)} "
    ...
    def get_state(self):
        ...
        state = {
            ...
            'target_zone_metadata': self.target_zone_metadata,
            'use_spillage_model': getattr(self, 'use_spillage_model', False),
            ...
            'target_visibility_mode': self.target_visibility_mode,
            'highway_visibility_mode': self.highway_visibility_mode,
            'dynamic_visibility_min_angle': self.angle_min_deg,
            'dynamic_visibility_max_angle': self.angle_max_deg,
            'target_zone_gate_factor': self.target_zone_gate_factor,
    ...
    def set_state(self, state):
        ...
        self.target_zone_metadata = dict(state.get('target_zone_metadata', {}))
        ...
        self.target_visibility_mode = state.get('target_visibility_mode', 'fixed')
        self.highway_visibility_mode = state.get('highway_visibility_mode', 'fixed')
        self.angle_min_deg = state.get('dynamic_visibility_min_angle', 30.0)
        self.angle_max_deg = state.get('dynamic_visibility_max_angle', 60.0)
        self.target_zone_gate_factor = state.get('target_zone_gate_factor', 1.0)
