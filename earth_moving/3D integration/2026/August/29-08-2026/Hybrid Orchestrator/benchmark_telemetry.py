...
def calculate_environment_metrics(
    ...
    return {
        ...
        "max_heat": max_heat,
        "heat_total": sum(heats),
        "heat_mean": sum(heats) / len(heats) if heats else 0.0,
        "positive_heat_cell_fraction": (
            sum(1 for heat in heats if heat > 0.0) / len(heats) if heats else 0.0
        ),
...
class BenchmarkTelemetry:
    ...
    SNAPSHOT_FIELDS = (
        ...
        "heat_max", "heat_mean", "heat_total", "positive_heat_cell_fraction",
        "heat_l1_change", "heat_changed_cell_fraction", "occupied_cell_count",
        "outside_centroid_x_m", "outside_centroid_y_m", "outside_rms_spread_m",
        "outside_mean_signed_target_distance_m",
    ...
    def __init__(
        ...
        self._previous_heat_by_cell: Optional[Dict[tuple[int, int], float]] = None
        self._closed = False
    ...
    def record_snapshot(
        ...
        serialized_cells = self._serialize_cells(env)
        current_heat = {
            (int(cell["x"]), int(cell["y"])): float(cell["heat"])
            for cell in serialized_cells
        }
        heat_l1_change = None
        heat_changed_fraction = None
        if self._previous_heat_by_cell is not None:
            keys = set(current_heat).union(self._previous_heat_by_cell)
            absolute_change = sum(
                abs(current_heat.get(key, 0.0) - self._previous_heat_by_cell.get(key, 0.0))
                for key in keys
            )
            previous_scale = sum(abs(value) for value in self._previous_heat_by_cell.values())
            heat_l1_change = absolute_change / max(1e-12, previous_scale)
            heat_changed_fraction = (
                sum(
                    1 for key in keys
                    if abs(current_heat.get(key, 0.0) - self._previous_heat_by_cell.get(key, 0.0)) > 1e-9
                ) / len(keys)
                if keys else 0.0
            )
        self._previous_heat_by_cell = current_heat

        outside_pebbles = [
            state for state in pebble_states.values() if not bool(state.get("in_target"))
        ]
        physical_weights = [
            float(state.get("material_mass", 1.0)) if mode == "mass" else 1.0
            for state in outside_pebbles
        ]
        physical_weight = sum(physical_weights)
        outside_centroid_x = outside_centroid_y = outside_rms = outside_mean_distance = 0.0
        if physical_weight:
            outside_centroid_x = sum(
                float(state["position"][0]) * weight
                for state, weight in zip(outside_pebbles, physical_weights)
            ) / physical_weight
            outside_centroid_y = sum(
                float(state["position"][1]) * weight
                for state, weight in zip(outside_pebbles, physical_weights)
            ) / physical_weight
            outside_rms = math.sqrt(sum(
                weight * (
                    (float(state["position"][0]) - outside_centroid_x) ** 2
                    + (float(state["position"][1]) - outside_centroid_y) ** 2
                )
                for state, weight in zip(outside_pebbles, physical_weights)
            ) / physical_weight)
            outside_mean_distance = sum(
                float(state.get("signed_target_distance_m", 0.0)) * weight
                for state, weight in zip(outside_pebbles, physical_weights)
            ) / physical_weight

        payload = {
            "schema": 3, "run_id": self.run_id, "snapshot_id": snapshot_id, "reason": str(reason),
            ...
            "heatmap_change": {
                "normalized_l1": heat_l1_change,
                "changed_cell_fraction": heat_changed_fraction,
            },
            "physical_distribution": {
                "outside_centroid_x_m": outside_centroid_x,
                "outside_centroid_y_m": outside_centroid_y,
                "outside_rms_spread_m": outside_rms,
                "outside_mean_signed_target_distance_m": outside_mean_distance,
            },
        }
        ...
        is_push_snapshot = str(reason).startswith(("before_push", "after_push"))
        write_image = self.capture_images and (
            bool(force_image) or (self.capture_push_images and is_push_snapshot)
        )
        if write_image:
            try:
        ...
        row = {
            ...
            **{key: metrics.get(key) for key in (
                ...
                "mean_distance_to_target_cells", "median_distance_to_target_cells", "p90_distance_to_target_cells",
                "spatial_concentration_score", "direct_target_feasible_fraction")},
            "heat_max": metrics.get("max_heat"),
            "heat_mean": metrics.get("heat_mean"),
            "heat_total": metrics.get("heat_total"),
            "positive_heat_cell_fraction": metrics.get("positive_heat_cell_fraction"),
            "heat_l1_change": heat_l1_change,
            "heat_changed_cell_fraction": heat_changed_fraction,
            "occupied_cell_count": metrics.get("occupied_cell_count"),
            "outside_centroid_x_m": outside_centroid_x,
            "outside_centroid_y_m": outside_centroid_y,
            "outside_rms_spread_m": outside_rms,
            "outside_mean_signed_target_distance_m": outside_mean_distance,
    ...
    def close(self, sim_time: float, outcome: str, final_progress: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            "schema": 3, "run_id": self.run_id, "outcome": outcome, "end_sim_time": float(sim_time),
