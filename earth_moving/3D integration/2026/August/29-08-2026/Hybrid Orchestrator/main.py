...
class NullSimulationVisualizer:
    """Minimal visualizer interface for Linux/headless experiment workers.

    Planning code may still publish a trajectory preview or replace the current
    2D environment.  Keeping those calls valid avoids mixing visualization
    decisions into planner behavior while eliminating the SDL window and draw
    cost from benchmark runs.
    """
    ...
    def set_trajectory_preview(self, *args, **kwargs):
        return None
...
def run_2d_env(env_radius, target_zone_radius, shovel_width, real_objects,
    ...
    """Build the 2D environment and optionally create the PyGame visualizer."""
    env = compute_2d_env(
        env_radius, target_zone_radius, shovel_width, real_objects,
        manual_mode, use_spillage_model, visualize_potential,
        target_zone=target_zone)
    visualizer = (
        SimulationVisualizer(env, screen_size=800)
        if enable_visualizer else NullSimulationVisualizer(env)
    )
