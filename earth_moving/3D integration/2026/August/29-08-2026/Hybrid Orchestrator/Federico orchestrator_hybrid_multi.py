...
from rover_profiles import ensure_profile_urdf
from pebble_profiles import (
    DEFAULT_MATERIAL_VALUE_MODE, annotate_material_cells,
    expand_positions_for_planning, generate_pebble_instances,
    normalize_distribution, normalize_material_mode, progress_from_positions,
    summarize_instances,
)
from target_zones import resolve_target_zone

PEBBLE_DEPLOYMENTS = frozenset({
    "uniform_area", "clustered", "near_target", "outer_ring",
})
...
def register_rover_kinematics(body_id, profile):
    """Bind physical drive dimensions to a loaded body."""
    if profile is None:
        _ROVER_KINEMATICS.pop(body_id, None)
        return
    geometry = profile.geometry
    _ROVER_KINEMATICS[body_id] = (
        float(geometry.wheel_radius),
        float(geometry.track_width),
        max(1.0, float(geometry.chassis_scale) ** 3),
    )
...
def set_wheel_velocities(body_id, left_joint, right_joint, v_cmd, w_cmd,
    ...
    """Set differential drive velocities using this body's configured geometry."""
    configured = _ROVER_KINEMATICS.get(body_id)
    if configured is not None:
        wheel_radius, track_width, torque_scale = configured
        max_torque *= torque_scale
...
def plan_for_agent(snapshot: WorldSnapshot) -> PlanResult:
    try:
        ...
        coord_converter = CoordinateConverter(
            snapshot.env_radius, snapshot.target_zone_radius, snapshot.shovel_width,
            target_zone=snapshot.target_zone)
...
class MultiAgentHybridOrchestrator:
    def __init__(self, env_radius=3.0, target_zone_radius=0.3, num_pebbles=50,
                 ...
                 phase1_tracking_point="shovel", rover_profiles=None,
                 material_value_mode=DEFAULT_MATERIAL_VALUE_MODE,
                 pebble_distribution=None, target_zone=None,
                 scenario_name="custom", headless=False,
                 fast_headless=False, pebble_deployment="uniform_area"):
        self.env_radius = env_radius
        self.target_zone = resolve_target_zone(target_zone, target_zone_radius)
        self.target_zone_radius = self.target_zone.bounding_radius
        self.scenario_name = str(scenario_name)
        self.num_pebbles = num_pebbles
        self.random_seed = random_seed
        self.shovel_width = shovel_width
        self.auto_mode = auto_mode
        self.headless = bool(headless or fast_headless)
        self.fast_headless = bool(fast_headless)
        self.material_value_mode = normalize_material_mode(material_value_mode)
        self.pebble_distribution = normalize_distribution(pebble_distribution)
        self.pebble_deployment = str(pebble_deployment).strip().lower()
        if self.pebble_deployment not in PEBBLE_DEPLOYMENTS:
            raise ValueError(
                f"pebble_deployment must be one of {sorted(PEBBLE_DEPLOYMENTS)}"
            )
        ...
        self.rover_profiles = list(rover_profiles or [None] * self.num_rovers)
        if len(self.rover_profiles) != self.num_rovers:
            raise ValueError("rover_profiles must contain one profile per initial pose")
        ...
        self.agents = []
        self.pebble_centers = []
        self.pebble_instances = []
        self.pebble_instances_by_id = {}
        self.live_pebble_instances = []
    ...
    def initialize(self):
        ...
        connection_id = p.connect(connection_mode)
        if connection_id < 0:
            raise RuntimeError(
                "PyBullet could not open a DIRECT connection"
                if self.headless else "PyBullet could not open a GUI connection"
            )
        ...
        # Target zone
        if self.target_zone.shape == "circle":
            tz_vis = p.createVisualShape(
                p.GEOM_CYLINDER, radius=self.target_zone.radius,
                length=0.01, rgbaColor=[1, 0, 0, 0.3])
        elif (
            self.target_zone.shape == "rectangle"
            and abs(self.target_zone.rotation_deg) <= 1e-12
        ):
            tz_vis = p.createVisualShape(
                p.GEOM_BOX,
                halfExtents=[0.5 * self.target_zone.width, 0.5 * self.target_zone.height, 0.005],
                rgbaColor=[1, 0, 0, 0.3])
        else:
            # Flat triangulated visual mesh supports ellipse, semicircle and
            # arbitrary concave/amorphous polygons without changing physics.
            vertices = []
            indices = []
            cx, cy = self.target_zone.center
            for triangle in self.target_zone.triangles_world():
                base_index = len(vertices)
                vertices.extend([
                    [float(x) - cx, float(y) - cy, 0.0]
                    for x, y in triangle
                ])
                indices.extend([base_index, base_index + 1, base_index + 2])
            if not indices:
                raise RuntimeError("target-zone visual triangulation produced no triangles")
            tz_vis = p.createVisualShape(
                p.GEOM_MESH,
                vertices=vertices,
                indices=indices,
        ...
        p.createMultiBody(
            baseMass=0, baseVisualShapeIndex=tz_vis,
            basePosition=[self.target_zone.center[0], self.target_zone.center[1], 0.005])
        ...
        for i, (x, y, yaw) in enumerate(self.initial_robot_poses):
            profile = self.rover_profiles[i]
            profile_urdf = ensure_profile_urdf(rover_urdf, profile) if profile is not None else rover_urdf
            chassis_scale = float(profile.geometry.chassis_scale) if profile is not None else 1.0
            body_id = p.loadURDF(
                profile_urdf, [x, y, 0.02 * chassis_scale], yaw_to_quat(yaw),
                globalScaling=chassis_scale)
            left_j, right_j = find_wheel_joints(body_id)
            register_rover_kinematics(body_id, profile)
            ...
            profile_label = profile.name if profile is not None else "legacy"
            shovel_width = profile.shovel_width if profile is not None else self.shovel_width
            chassis_scale = profile.geometry.chassis_scale if profile is not None else 1.0
            capacity = profile.capacity_objects if profile is not None else "legacy"
            mass_capacity = profile.capacity_mass if profile is not None else "legacy"
            print(
                f"  Rover {i} at ({x:.2f}, {y:.2f}), type={profile_label}, "
                f"scale={chassis_scale:.2f}, shovel={shovel_width:.2f}m, "
                f"capacity_objects={capacity}, capacity_mass={mass_capacity}"
            )

        # Pebbles
        pebble_urdf = os.path.join(here, "pebbles.urdf")
        np.random.seed(self.random_seed)
        deployment_rng = np.random.RandomState(self.random_seed)
        self.pebble_ids = []  # Store IDs to query positions later
        self.pebble_instances = list(generate_pebble_instances(
            self.num_pebbles, self.random_seed, self.pebble_distribution))
        cluster_centers = []
        if self.pebble_deployment == "clustered":
            for _ in range(3):
                for _attempt in range(10000):
                    radius = 0.75 * self.env_radius * math.sqrt(deployment_rng.rand())
                    angle = 2.0 * math.pi * deployment_rng.rand()
                    candidate = (radius * math.cos(angle), radius * math.sin(angle))
                    if self.target_zone.signed_distance_world(*candidate) >= 0.35:
                        cluster_centers.append(candidate)
                        break
                else:
                    raise RuntimeError("could not sample an outside cluster center")

        def sample_deployment_position():
            for _attempt in range(20000):
                if self.pebble_deployment == "outer_ring":
                    r_min = 0.70 * self.env_radius
                    radius = math.sqrt(
                        deployment_rng.rand() * (self.env_radius ** 2 - r_min ** 2)
                        + r_min ** 2
                    )
                    angle = 2.0 * math.pi * deployment_rng.rand()
                    candidate = (radius * math.cos(angle), radius * math.sin(angle))
                elif self.pebble_deployment == "clustered":
                    center = cluster_centers[int(deployment_rng.randint(len(cluster_centers)))]
                    candidate = (
                        center[0] + float(deployment_rng.normal(0.0, 0.22)),
                        center[1] + float(deployment_rng.normal(0.0, 0.22)),
                    )
                else:
                    radius = self.env_radius * math.sqrt(deployment_rng.rand())
                    angle = 2.0 * math.pi * deployment_rng.rand()
                    candidate = (radius * math.cos(angle), radius * math.sin(angle))
                if math.hypot(*candidate) > self.env_radius:
                    continue
                distance = self.target_zone.signed_distance_world(*candidate)
                if distance <= 1e-9:
                    continue
                if self.pebble_deployment == "near_target" and not (0.10 <= distance <= 0.75):
                    continue
                return candidate
            raise RuntimeError(
                f"could not sample a {self.pebble_deployment} pebble outside the target zone"
            )

        for pebble_index in range(self.num_pebbles):
            pebble = self.pebble_instances[pebble_index]
            if (
                self.pebble_deployment == "uniform_area"
                and self.target_zone.shape == "circle"
                and self.target_zone.center == (0.0, 0.0)
            ):
                # Preserve the historical circular scenario's deterministic spawn sequence.
                r_min = self.target_zone.radius
            ...
            elif self.pebble_deployment == "uniform_area":
                for _ in range(10000):
                    r = self.env_radius * math.sqrt(np.random.rand())
                    phi = 2 * math.pi * np.random.rand()
                    px, py = r * math.cos(phi), r * math.sin(phi)
                    if not self.target_zone.contains_world(px, py):
                        break
                else:
                    raise RuntimeError("could not sample a pebble outside the target zone")
            else:
                px, py = sample_deployment_position()
            z_height = 0.005 * float(pebble.visual_scale)
            bid = p.loadURDF(
                pebble_urdf, [px, py, z_height], useFixedBase=False,
                globalScaling=float(pebble.visual_scale))
            p.changeDynamics(bid, -1, lateralFriction=0.8)
            p.changeVisualShape(bid, -1, rgbaColor=pebble.color)
            self.pebble_centers.append((px, py))
            self.pebble_ids.append(bid)
            self.pebble_instances_by_id[bid] = pebble

        pebble_summary = summarize_instances(self.pebble_instances)
        print(
            f"  {len(self.pebble_centers)} mixed pebbles spawned "
            f"(material_mass={pebble_summary['total_material_mass']}, "
            f"mode={self.material_value_mode}, profiles={pebble_summary['profiles']})"
            f", deployment={self.pebble_deployment}"
        )

        # Coordinate converter. Include room behind boundary pebbles so their
        # approach gates exist in both the canonical map and rover overlays.
        print("\n[2/3] Setting up 2D environment...")
        initial_map_radius = self.calculate_dynamic_env_radius(
            self.pebble_centers, safety_margin=0.75,
        )
        self.coord_converter = CoordinateConverter(
            initial_map_radius, self.target_zone_radius, self.shovel_width,
            target_zone=self.target_zone)

        # 2D environment
        physical_pebbles_3d = [
            (x, y, 0.005 * float(instance.visual_scale))
            for (x, y), instance in zip(self.pebble_centers, self.pebble_instances)
        ]
        planning_pebbles_3d = expand_positions_for_planning(
            physical_pebbles_3d, self.pebble_instances, self.material_value_mode)
        self.env_2d, self.visualizer = run_2d_env(
            ...
            visualize_potential=SIMULATION_CONFIG['visualize_potential'],
            target_zone=self.target_zone,
            enable_visualizer=not self.headless)
        annotate_material_cells(
            self.env_2d, self.coord_converter,
            self.pebble_centers, self.pebble_instances)
        self.env_2d.material_value_mode = self.material_value_mode
        for agent in self.agents:
            agent["coord_converter"] = self.coord_converter
    ...
    def run(self):
        ...
        shutdown_hook = getattr(self, "_before_runtime_shutdown", None)
        if callable(shutdown_hook):
            try:
                shutdown_hook()
            except Exception as exc:
                print(f"[SHUTDOWN] Final snapshot failed: {exc!r}")
        pygame.quit()
        if self.executor:
            self.executor.shutdown(wait=False, cancel_futures=True)
        p.disconnect()

    def _run_headless(self, sim_dt):
        """Run fixed-step physics without GUI/Pygame timing dependencies.

        The 20 Hz controller and 240 Hz PyBullet relationship is identical to
        the interactive runner.  ``fast_headless`` removes real-time sleeping;
        ordinary headless mode remains paced for parity diagnostics.
        """
        control_dt = 0.05
        physics_steps_per_control = int(round(control_dt / sim_dt))
        wall_started = time.perf_counter()
        control_steps = 0
        print(
            "\nStarting headless execution loop "
            f"(mode={'fast' if self.fast_headless else 'realtime'})..."
        )
        try:
            while self.running and p.isConnected():
                ...
                for _ in range(physics_steps_per_control):
                    p.stepSimulation()
                self._step_agents(control_dt)
                self._poll_planning_results()
                if self.auto_mode:
                    self._handle_auto_mode()
                control_steps += 1

                if self.fast_headless:
                    # Yield occasionally so background map/planning workers can
                    # complete even when simulated time advances very quickly.
            ...
            simulated = float(getattr(self, "_sim_time", 0.0))
            print(
                f"[HEADLESS] simulated={simulated:.2f}s wall={elapsed:.2f}s "
                f"speed={simulated / elapsed:.2f}x"
            )

    def _update_visualizer(self):
        """Passes 3D multi-agent state down to PyGame for dual path rendering"""
        class DummyAgentVis:
            def __init__(self, pos, agent_id, color):
                self.position = pos
        ...
        for i, agent in enumerate(self.agents):
            ...
            vis_agents.append(DummyAgentVis((cx, cy), agent["id"], agent["color"]))
            vis_colors[agent["id"]] = agent["color"]
            
            # Extract planned paths if any
            if agent.get('selection'):
                path_info = agent['selection']['path_info']['path']
                vis_paths[agent["id"]] = [(c.x, c.y) for c in path_info]
                
        self.visualizer.update_agents(vis_agents)
        self.visualizer.update_agent_colors(vis_colors)
        ...
        for agent in self.agents:
            ...
            with self._reservation_lock:
                if res.best_reserved.intersection(self.global_reserved_cells):
                    print(f"[PLAN] {agent['id']} plan conflicts with updated reservations ג€” replanning.")
                    agent["state"] = "IDLE"
                    continue
            ...
            agent["coord_converter"] = CoordinateConverter(
                res.env_radius, self.target_zone_radius, self.shovel_width,
                target_zone=self.target_zone)
    ...
    def calculate_dynamic_env_radius(self, pebble_centers, safety_margin=0.75):
        """Keep every live pebble plus an approach gate inside the shared planning map."""
        max_distance = 0.0
        ...
        nominal_radius = float(self.env_radius)
        dynamic_radius = max(
            nominal_radius,
            max_distance + max(0.0, float(safety_margin)),
            float(self.target_zone.bounding_radius) + 0.1,
        )
        expansion_cap = nominal_radius + max(
            2.0, float(getattr(self, "navigation_outside_margin", 0.0))
        )
        dynamic_radius = min(dynamic_radius, expansion_cap)
        ...
        if self._last_reported_env_radius is None or abs(dynamic_radius - self._last_reported_env_radius) > 1e-3:
            ...
            print(f"  - Nominal radius: {nominal_radius:.3f}m")
            print(f"  - Shared planning radius: {dynamic_radius:.3f}m")
    ...
    def _snapshot_pebbles_xy(self) -> np.ndarray:
        ...
        for bid in self.pebble_ids:
            ...
            if pos[2] > -0.5:
                ...
                instances.append(self.pebble_instances_by_id[bid])
        with self._world_state_lock:
            self.pebble_centers = list(pts)
            self.live_pebble_instances = list(instances)
    ...
    def _get_live_pebble_instances(self):
        with self._world_state_lock:
            return list(self.live_pebble_instances or self.pebble_instances)

    def _get_material_progress(self):
        with self._world_state_lock:
            paired = list(zip(self.pebble_centers, self.live_pebble_instances or self.pebble_instances))
        return progress_from_positions(paired, target_zone=self.target_zone)
    ...
    def _step_sync(self, agent):
        ...
        # Do not call clear_trajectory_preview() globally ג€” other agents may still be navigating.
