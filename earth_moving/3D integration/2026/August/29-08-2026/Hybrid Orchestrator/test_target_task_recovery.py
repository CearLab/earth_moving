...
class TargetTaskRecoveryTests(unittest.TestCase):
    ...
    def test_straight_comparison_offers_one_segment_for_every_source(self):
        overlay = build_profile_overlay_snapshot(
            3,
            self.env,
            self.converter,
            ROVER_TYPES["small"],
            material_value_mode="count",
            target_path_mode="straight_nearest",
            target_candidate_value_mode="source_only",
        )
        target_candidates = [
            candidate for candidate in overlay.candidates
            if candidate.task_type == "target"
        ]
        self.assertEqual(
            {candidate.source.key for candidate in target_candidates},
            {source.key for source in overlay.sources},
        )
        for candidate in target_candidates:
            self.assertEqual(candidate.path_info["guide_mode"], "straight_nearest_boundary")
            self.assertEqual(candidate.path_info["target_path_mode"], "straight_nearest")
            self.assertEqual(candidate.path_info["target_candidate_value_mode"], "source_only")
            self.assertEqual(len(candidate.path_info["world_path"]), 2)
